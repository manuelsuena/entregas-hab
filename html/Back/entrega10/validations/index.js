...
const searchSchema = Joi.string()
  ...
  .error(
    generateError(
      'El campo de búsqueda debe ser más de 2 caracteres',
      400
    )
  );
...
const emailSchema = Joi.string()
  .email()
  .required()
  .error(generateError('El campo email debe ser un email formado correctamente', 400));
...
const entrySchema = Joi.object().keys({
  ...
  description: Joi.string()
    ...
    .error(
      generateError(
        'El campo description es obligatorio y no puede tener más de 1000 caracteres',
        400
      )
    )
});
...
const editPasswordUserSchema = Joi.object().keys({
  ...
  newPasswordRepeat: Joi.any()
    ...
    .error(generateError('Las passwords deben ser iguales', 400))
});
