...
async function newUser(req, res, next) {
  let connection;
  try {
    // Validar body payload
    await userSchema.validateAsync(req.body);

    connection = await getConnection();
    const { email, password } = req.body;

    // Compruebe si el correo electrónico del usuario ya está en la base de datos
    const [
      existing
    ...
    try {
      await sendEmail({
        ...
        content: `Para Validar tu cuenta de usuario debe pega esta url en tu navegador: ${validationURL}`
      });
    } catch (error) {
      console.error(error.response.body);
      throw new Error(
        'Error en el envío de mail. Inténtalo de nuevo.'
      );
    }
    ...
    res.send({
      staus: 'ok',
      message:
        'Usuario registrado. Mira la carpeta del SPAM. Activa la cuenta'
    });
  } catch (error) {
    next(error);
  } finally {
...
async function updatePasswordUser(req, res, next) {
  ...
  try {
    ...
    res.send({
      status: 'ok',
      message:
        'Cambio de password realizado correctamente. Haz login de nuevo para conseguir un nuevo token válido.'
    });
  } catch (error) {
    next(error);
  } finally {
